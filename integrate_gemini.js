...
// 1. Rewrite .env configuration
try {
  let env = fs.readFileSync(envFile, 'utf8');
  env = env.replace(/ANTHROPIC_API_KEY/g, 'GEMINI_API_KEY');
  fs.writeFileSync(envFile, env, 'utf8');
  console.log('- Updated env file to GEMINI_API_KEY');
} catch (e) {
  console.error('Env update error:', e.message);
}

// 2. Rewrite App.jsx frontend references
try {
  let app = fs.readFileSync(appFile, 'utf8');
  
  // Replace localStorage keys
  app = app.replace(/anthropic_key/g, 'gemini_key');
  
  // Replace placeholders and labels
  app = app.replace(/Configure Anthropic API Key/g, 'Configure Gemini API Key');
  app = app.replace(/API Key Saved/g, 'Gemini Key Saved');
  app = app.replace(/Enter API Key/g, 'Enter Gemini Key');
  app = app.replace(/Anthropic credentials required/g, 'Gemini credentials required');
  app = app.replace(/Enter your standard `claude-sonnet-4-20250514` API Key below/g, 'Enter your standard Google AI Studio Gemini API Key below');
  app = app.replace(/sk-ant-\.\.\./g, 'AIzaSy...');
  app = app.replace(/Please ensure your Anthropic API Key is correct/g, 'Please ensure your Gemini API Key is correct');
  app = app.replace(/Model: Claude 3.5 Sonnet/g, 'Model: Gemini 1.5 Flash');
  app = app.replace(/Model: Claude 3\.5 Sonnet/g, 'Model: Gemini 1.5 Flash');

  fs.writeFileSync(appFile, app, 'utf8');
  console.log('- Successfully updated App.jsx frontend to Gemini');
} catch (e) {
  console.error('App.jsx update error:', e.message);
}

// 3. Rewrite server.js backend completely to utilize @google/generative-ai
const newServerJsCode = `import express from 'express';
...
try {
  fs.writeFileSync(serverFile, newServerJsCode, 'utf8');
  console.log('- Successfully replaced server.js with Google AI Studio Gemini API integration');
} catch (e) {
  console.error('server.js rewrite error:', e.message);
}
